...
**TUN 异常退出后 DNS 坏了** — macOS: 运行 `sudo ./restore-dns.sh` 恢复，或手动执行 `sudo networksetup -setdnsservers Wi-Fi Empty && dscacheutil -flushcache`。Windows: 在管理员 PowerShell 中运行 `Set-DnsClientServerAddress -InterfaceAlias "Wi-Fi" -ResetServerAddresses`。
